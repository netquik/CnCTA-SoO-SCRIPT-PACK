...
(function () {
	var TransferAll_main = function () {
		...
		function createTransferAll() {
			try {
				...
				var btnTransferAll = new webfrontend.ui.SoundButton("Transfer All").set({
					width: 80,
					enabled: false
				});
				...
				transferWindow[3].add(btnTransferAll, {
					right: 2,
					top: 100
				});
				transferWindow[3].add(chkbxConfirm, {
					right: 68,
					top: 104
				});
			} catch (e) {
			...
			try {
				if (transferQueue.length > 0) {
					...
					ClientLib.Net.CommunicationManager.GetInstance().SendCommand("SelfTrade", {
						...
						sourceCityId: sourceCity.get_Id(),
						resourceType: resType,
						amount: resAmount
					}, phe.cnc.Util.createEventDelegate(ClientLib.Net.CommandResult, this, transferResult), null, true);
				}
			} catch (e) {
	...
	try {
		...
		TransferAll.textContent = "(" + TransferAll_main.toString() + ")();";
