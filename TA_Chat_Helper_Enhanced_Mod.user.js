...
// type: /chelp in any text box and hit <space> for a list of commands
        ...
        try {
            function createchatHelper() {
                ...
                document.onkeyup = function (kEv) {
                    // try fixing input
                    if ((kEv.target.type === "textarea" || kEv.target.type === "text") && kEv.target.value !== '') {
                        window.inputchat = kEv.target.value;

                        if (kEv.keyCode === 32) {
                            // /chelp dialog
                            if (window.inputchat.length === 7 && window.inputchat.match(/^(\/chelp\s?)/) !== null) {
                                chlog("clearing input field", 1);
                                kEv.target.value = "";
                                showHelp();
                                return false;
                            } else {
                                preautotag(kEv, window.inputchat);
                            }
                        }
                    }
                    clearTimeout(timer);
                ...
                function autoTag(inputField, inText) {
                    ...
                    inText = inText.replace(/(\[coords\])*([0-9]{3,4})[:|.]([0-9]{3,4})([:|.]\w+)?(\[\/coords\])*/gi, function () {
                        if (!arguments[7].startsWith("http://cnc-map") && !arguments[7].startsWith("[url]")) {
                ...
                function preautotag2(kEv, txt) {
                    // chat or message?
                    var inputchat;
                    if (txt !== '') {
                        inputchat = txt;
                    } else {
                        inputchat = kEv.target.value;
                    }
                    return inputchat;
                }


                function preautotag(kEv, txt) {
                    var inputField = kEv.target;
                    var inText = preautotag2(kEv, txt);
                ...
                document.onkeydown = function (kEv) {
                    ...
                    if (kEv.keyCode === 13) {
                        if (kEv.target.type === "text" && kEv.target.value === '') {
                            // do nothing 
                        } else if (kEv.target.type === "textarea" && kEv.target.value !== '') {
                            preautotag(kEv, '');
                        }
